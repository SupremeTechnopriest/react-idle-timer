...
{/* cspell:disable */}
- [timeout](/docs/api/props#timeout): The time before the `onPrompt` callback will be fired.
- [promptTimeout](/docs/api/props#prompttimeout): How long the prompt should last before going idle in milliseconds.
...
- [getRemainingTime()](/docs/api/methods#getremainingtime): Will return the time until idle when active and the time until the prompt expires when prompted.
{/* cspell:enable */}
